...
export default class GetAllCommand extends Command {
  ...
  description =
    "Clones all repositories of a given user (excluding those that already exist)";

  async callback(options: Object, username: string) {
    let repositories;
    try {
      repositories = await getGHAllPages(
    ...
    await this.tasks(
      repositories.map(repository => ({
        ...
        callback: async (ctx, task) => {
          ...
          const { code: exitCode } = await this.spawn(
            "git",
            ["clone", repository.ssh_url, projectPath],
            {
              stdio: "ignore"
            }
          );
          if (exitCode !== 0) {
            throw new CLIError(
              `Failed to clone repository: ${username}/${repository.name}`
            );
          }
        }
      })),
